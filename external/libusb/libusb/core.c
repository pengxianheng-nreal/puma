...
static int driver_libusb_log_level = 0;
#endif
	...
	enum libusb_option option, ...)
{
	...
	case LIBUSB_OPTION_LOG_LEVEL:
		...
		driver_libusb_log_level = arg;
		//XREAL_LOG("driver_libusb_log_level=%d\n",driver_libusb_log_level);
#if defined(ENABLE_LOGGING) && !defined(ENABLE_DEBUG_LOGGING)
		if (ctx && !ctx->debug_fixed)
	...
	const char *function, const char *format, va_list args)
{
	...
	UNUSED(ctx);
#else
	enum libusb_log_level ctx_level = driver_libusb_log_level;


	//XREAL_LOG("ctx_level=%d,level=%d\n",ctx_level,level);
	// USBI_GET_CONTEXT(ctx);
	// if (ctx)
	// 	ctx_level = ctx->debug;
	// else
...
DEFAULT_VISIBILITY const char * LIBUSB_CALL libusb_error_name(int error_code)
{
	...
	case 0:
		return "SUCCESS / TRANSFER_COMPLETED";
	default:
