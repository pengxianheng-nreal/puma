...
#define UVC_ERROR(format, ...) __android_log_print(ANDROID_LOG_ERROR, "XREAL", format "\n", ##__VA_ARGS__)
#else
#include <libgen.h>
...
#if defined(__ANDROID__)
#include <android/log.h>
#define UVC_PROFILING(format, ...) __android_log_print(ANDROID_LOG_INFO, "XREAL", format "\n", ##__VA_ARGS__)
#define UVC_DEBUG(format, ...) __android_log_print(ANDROID_LOG_INFO, "XREAL", format "\n", ##__VA_ARGS__)
#define UVC_ENTER()
#define UVC_EXIT(code)
#define UVC_EXIT_VOID()
#else
#include <libgen.h>
#define UVC_PROFILING(format, ...)
 ...
 */
#define LIBUVC_NUM_TRANSFER_BUFS 30

/*
  Here for iso transfer, we apply for small contiguous memory blocks 
  to avoid out of memory because usbfs does not have large contiguous memory
  The previous default value in uvc is 32, and the size of each packet is 12288 bytes, 
  so the size of a transfer memory block is 384k bytes, which is too large.
  We change to 5 here, the corresponding continuous memory is 60k bytes
 */
#define LIBUVC_NUM_ISO_PKG_PER_TRANSFER 5
...
struct uvc_stream_handle {
  ...
  uvc_frame_copyframe_t *user_copyframe;
  void *user_ptr;
