...
uvc_error_t uvc_write_register(uvc_device_handle_t *devh, int32_t address, uint8_t value) {
    ...
    while (retry--) {
        ...
        ret = libusb_control_transfer(
                ...
                0);
        //printf("retry++\n");
        //printf("write ret code:%x,%x\n",data[0],data[1]);
        if (data[0] == 0x56 && data[1] == 0x50)
            return UVC_SUCCESS;
...
uvc_error_t
uvc_write_sccb(uvc_device_handle_t *devh, uint8_t ch, uint8_t slave_address, int32_t address, uint8_t write_data) {
    uint8_t data[1024];
    int i;
    uvc_error_t ret;
    uint8_t data_addr[4];
    ...
    //printf("sccb write ret code:%x,%x\n", data[0], data[1]);
    //printf("read---------------------\n");
    if (data[0] == 0x56 && data[1] == 0x50)
        return UVC_SUCCESS;
    else
...
uvc_error_t
uvc_write_sccb_retry(uvc_device_handle_t *devh, uint8_t ch, uint8_t slave_address, int32_t address, uint8_t write_data, int retry) {
    uint8_t data[1024];
    int i;
    uvc_error_t ret;
    uint8_t data_addr[4];
...
uvc_error_t
uvc_read_sccb(uvc_device_handle_t *devh, uint8_t ch, uint8_t slave_address, int32_t address, uint8_t *ret_data) {
    uint8_t data[1024];
    int i;
    uvc_error_t ret;
    uint8_t data_addr[4];
