...
//                printf("SampleCB() latestBufferLength is %d.\n",latestBufferLength);
                ...
                LOG_ERROR("ERROR: SampleCB() - buffer sizes do not match, latestBufferLength = {}, numBytes = {}",
                    latestBufferLength, numBytes);
            }
        }
...
//            }
//        }
                LOG_INFO("bufferCB lens is {}.",BufferLen);
...
videoDevice::videoDevice(){
     ...
     tryWidth           = 640;
     tryHeight          = 480;
     tryVideoType = MEDIASUBTYPE_YUY2;
     ...
     autoReconnect      = false;
     requestedFrameTime = 10000000/30;
...
void videoDevice::destroyGraph(){
    ...
    while (hr == NOERROR)
    {
        ...
        if (FAILED(hr)) { if(verbose)LOG_INFO("SETUP: pGraph->EnumFilters() failed."); return; }
        ...
        if (pEnum->Next(1, &pFilter, &cFetched) == S_OK)
        {
            ...
            if(verbose)LOG_INFO("SETUP: removing filter {}...", buffer);
            hr = pGraph->RemoveFilter(pFilter);
            if (FAILED(hr)) { if(verbose)printf("SETUP: pGraph->RemoveFilter() failed."); return; }
            if(verbose)LOG_INFO("SETUP: filter removed {}",buffer);
...
videoDevice::~videoDevice(){
    ...
    if(setupStarted){ if(verbose)LOG_INFO("SETUP: Disconnecting device {}", myID); }
    else{
        if(sgCallback){
            sgCallback->Release();
            delete sgCallback;
    ...
    if( (sgCallback) && (pGrabber) )
    {
        ...
        if(verbose)LOG_INFO("SETUP: freeing Grabber Callback");
        sgCallback->Release();
    ...
    {
        HR = pControl->Pause();
        if (FAILED(HR)) if(verbose)LOG_INFO("ERROR - Could not pause pControl");

        HR = pControl->Stop();
        if (FAILED(HR)) if(verbose)LOG_INFO("ERROR - Could not stop pControl");
    }
    ...
    if( (pDestFilter) ){        if(verbose)LOG_INFO("SETUP: freeing Renderer");
                                (pDestFilter)->Release();
                                (pDestFilter) = 0;
    }
    if( (pVideoInputFilter) ){  if(verbose)LOG_INFO("SETUP: freeing Capture Source");
                                (pVideoInputFilter)->Release();
                                (pVideoInputFilter) = 0;
    }
    if( (pGrabberF) ){          if(verbose)LOG_INFO("SETUP: freeing Grabber Filter");
                                (pGrabberF)->Release();
                                (pGrabberF) = 0;
    }
    if( (pGrabber) ){           if(verbose)LOG_INFO("SETUP: freeing Grabber");
                                (pGrabber)->Release();
                                (pGrabber) = 0;
    }
    if( (pControl) ){           if(verbose)LOG_INFO("SETUP: freeing Control");
                                (pControl)->Release();
                                (pControl) = 0;
    }
    if( (pMediaEvent) ){        if(verbose)LOG_INFO("SETUP: freeing Media Event");
                                (pMediaEvent)->Release();
                                (pMediaEvent) = 0;
    }
    if( (streamConf) ){         if(verbose)LOG_INFO("SETUP: freeing Stream");
                                (streamConf)->Release();
                                (streamConf) = 0;
    }

    if( (pAmMediaType) ){       if(verbose)LOG_INFO("SETUP: freeing Media Type");
                                MyDeleteMediaType(pAmMediaType);
    }

    if((pMediaEvent)){
            if(verbose)LOG_INFO("SETUP: freeing Media Event");
            (pMediaEvent)->Release();
            (pMediaEvent) = 0;
    }
    ...
    if( (pCaptureGraph) ){      if(verbose)LOG_INFO("SETUP: freeing Capture Graph");
                                (pCaptureGraph)->Release();
                                (pCaptureGraph) = 0;
    }
    if( (pGraph) ){             if(verbose)LOG_INFO("SETUP: freeing Main Graph");
                                (pGraph)->Release();
                                (pGraph) = 0;
    }
    ...
    if(verbose)LOG_INFO("SETUP: Device {} disconnected and freed",myID);
}
...
videoInput::videoInput(){
    ...
    if(verbose)LOG_INFO("***** VIDEOINPUT LIBRARY - {} - TFW07 *****",VI_VERSION);
...
bool videoInput::setupDevice(int deviceNumber, int w, int h){
    ...
    setAttemptCaptureSize(deviceNumber,w,h,MEDIASUBTYPE_YUY2);
    if(setup(deviceNumber))return true;
    return false;
}
...
int videoInput::listDevices(bool silent,bool need_get_support_size){

    //COM Library Intialization
    comInit();

    if(!silent)LOG_INFO("VIDEOINPUT SPY MODE!");
    ...
    if (SUCCEEDED(hr))
    {
       ...
       if(hr == S_OK){
             ...
             if(!silent)LOG_INFO("SETUP: Looking For Capture Devices");
            ...
            while (pEnum->Next(1, &pMoniker, NULL) == S_OK){
                ...
                if (SUCCEEDED(hr)){
                    ...
                    if(need_get_support_size) {
                        std::string deviceName = deviceNames[deviceCounter];
                        SupportSizeVector support_sizes; 
                        GetSupportSize(pMoniker,deviceName, support_sizes);
                        cameras_support_sizes_.insert(std::make_pair(deviceName,support_sizes));
                    }
                    if(!silent)LOG_INFO("SETUP: {}) {}",deviceCounter, deviceNames[deviceCounter]);
                }
         ...
         if(!silent)LOG_INFO("SETUP: {} Device(s) found", deviceCounter);
    }
...
void videoInput::GetSupportSize(IMoniker* pMoniker,const std::string& device_name,SupportSizeVector& support_sizes)
{
     ULONG cFetched;
    ...
    while (pEnum->Next(1, &pPin, &cFetched) == S_OK) {
        ...
        if (pInfo.pFilter != nullptr) pInfo.pFilter->Release();

        IEnumMediaTypes* pEnumMT = nullptr;
        pPin->EnumMediaTypes(&pEnumMT);

        AM_MEDIA_TYPE* pmt = nullptr;
        while (pEnumMT->Next(1, &pmt, &cFetched) == S_OK) {
            if (pmt->formattype == FORMAT_VideoInfo) {
                VIDEOINFOHEADER* pvi = (VIDEOINFOHEADER*)pmt->pbFormat;
                support_sizes.emplace_back(pvi->bmiHeader.biWidth,pvi->bmiHeader.biHeight);
/*                LOG_INFO("GetSupportSize device_name:{},SETUP: Width: {}, Height: {}", device_name.c_str(),pvi->bmiHeader.biWidth, pvi->bmiHeader.biHeight);*/
            }
    ...
    if(isDeviceSetup(id)){
        if(bCallback){
            ...
            if( result != WAIT_OBJECT_0) {
                LOG_ERROR("getPixels WaitForSingleObject result = {}", result);
                return false;
            }
            ...
            EnterCriticalSection(&VDList[id]->sgCallback->critSection);
                ...
                if (frame_copy != nullptr) {
                    int dataBytes = width * height * IMAGE_NUM ;
                    frame_copy(src, dst, dataBytes, width, height, IMAGE_NUM);
                }
                else {
            ...
            if(hr==S_OK){
                ...
                if (numBytes == bufferSize){
                    ...
                    if(verbose)LOG_ERROR("ERROR: GetPixels() - bufferSizes do not match!\n");
                }
            }else{
                if(verbose)LOG_ERROR("ERROR: GetPixels() - Unable to grab frame for device %i\n", id);
            }
        }
    }
...
bool videoInput::isFrameNew(int id){
    ...
    if(freeze && VDList[id]->autoReconnect){
        if(verbose)LOG_ERROR("ERROR: Device seems frozen - attempting to reconnect\n");
        if( !restartDevice(VDList[id]->myID) ){
            if(verbose)LOG_ERROR("ERROR: Unable to reconnect to device\n");
        }else{
            if(verbose)LOG_INFO("SUCCESS: Able to reconnect to device\n");
        }
    }
...
std::map<std::string, SupportSizeVector> videoInput::cameras_support_sizes_;
...
bool videoInput::setup(int deviceNumber){
    ...
    {
        if(verbose)LOG_ERROR("SETUP: device[{}] not found - you have {} devices available", deviceNumber, devicesFound);
        if(devicesFound>=0) if(verbose)LOG_ERROR("SETUP: this means that the last device you can use is device[{}]",  devicesFound-1);
        return false;
    }

    if(VDList[deviceNumber]->readyToCapture)
    {
        if(verbose)LOG_ERROR("SETUP: can't setup, device {} is currently being used",VDList[deviceNumber]->myID);
        return false;
    }
...
void videoInput::processPixels(unsigned char * src, unsigned char * dst, int width, int height, bool bRGB, bool bFlip){
    ...
    int widthInBytes = width * IMAGE_NUM;
...
int videoInput::start(int deviceID, videoDevice *VD){
    ...
    if(verbose)LOG_INFO("SETUP: Setting up device {}",deviceID);
    ...
    {
        if(verbose)LOG_ERROR("ERROR - Could not create the Filter Graph Manager");
        return hr;
    }
    ...
    if (FAILED(hr))
    {
        if(verbose)LOG_ERROR("ERROR - Could not add the graph builder!");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr))
    {
        if(verbose)LOG_ERROR("ERROR - Could not set filtergraph");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr))
    {
        if(verbose)LOG_ERROR("ERROR - Could not create the Media Control object");
           stopDevice(deviceID);
        return hr;
    }
    ...
    if (SUCCEEDED(hr)){
        if(verbose)LOG_INFO("SETUP: {}", VD->nDeviceName);
        hr = VD->pGraph->AddFilter(VD->pVideoInputFilter, VD->wDeviceName);
    }else{
        if(verbose)LOG_ERROR("ERROR - Could not find specified video device");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if(FAILED(hr)){
        if(verbose)LOG_INFO("SETUP: Couldn't find preview pin using SmartTee");
    }else{
    ...
    if(VD->useCrossbar)
    {
        if(verbose)LOG_INFO("SETUP: Checking crossbar");
    ...
    if(FAILED(hr)){
        if(verbose)LOG_ERROR("ERROR: Couldn't config the stream!");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if(FAILED(hr)){
        if(verbose)LOG_ERROR("ERROR: Couldn't getFormat for pAmMediaType!");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if(customSize){
        if(verbose)    LOG_INFO("SETUP: Default Format is set to {} by {}", currentWidth, currentHeight);

        char guidStr[8];
            // try specified format and size
            getMediaSubtypeAsString(VD->tryVideoType, guidStr);
            if(verbose)LOG_INFO("SETUP: trying specified format {} @ {} by {}", guidStr, VD->tryWidth, VD->tryHeight);

            if( setSizeAndSubtype(VD, VD->tryWidth, VD->tryHeight, VD->tryVideoType) ){
                VD->setSize(VD->tryWidth, VD->tryHeight);
                foundSize = true;
                LOG_INFO("SETUP: trying specified format {} @ {} by {} successfully.",
                ...
                for(int i = 0; i < VI_NUM_TYPES; i++){
                    ...
                    if(verbose)LOG_INFO("SETUP: trying format {} @ {} by {}", guidStr, VD->tryWidth, VD->tryHeight);
        ...
        if( foundSize == false ){
            if( verbose )LOG_INFO("SETUP: couldn't find requested size - searching for closest matching size");
            ...
            if( closestWidth != -1 && closestHeight != -1){
                ...
                if(verbose)LOG_INFO("SETUP: closest supported size is {} @ {} {}", guidStr, closestWidth, closestHeight);
    ...
    if (FAILED(hr)){
        if(verbose)LOG_ERROR("Could not Create Sample Grabber - CoCreateInstance()");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)){
        if(verbose)LOG_ERROR("Could not add Sample Grabber - AddFilter()");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)){
        if(verbose)LOG_ERROR("ERROR: Could not query SampleGrabber");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if(bCallback){
        ...
        if (FAILED(hr)){
            if(verbose)LOG_ERROR("ERROR: problem setting callback");
            stopDevice(deviceID);
            return hr;
        }else{
            if(verbose)LOG_INFO("SETUP: Capture callback set");
        }
    }
    ...
    if(VD->streamConf){
        ...
        if(verbose)LOG_ERROR("ERROR: connecting device - prehaps it is already being used?");
        stopDevice(deviceID);
        return S_FALSE;
    }
    ...
    if (FAILED(hr)){
        if(verbose)LOG_ERROR("ERROR: Could not create filter - NullRenderer");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)){
        if(verbose)LOG_ERROR("ERROR: Could not add filter - NullRenderer");
        stopDevice(deviceID);
        return hr;
    }
    ...
    if (FAILED(hr)){
        if(verbose)LOG_ERROR("ERROR: Could not connect pins - RenderStream()");
        stopDevice(deviceID);
        return hr;
    }
    ...
    //EXP - lets try setting the sync source to null - and make it run as fast as possible
    {
        ...
        if (FAILED(hr)){
            if(verbose)LOG_ERROR("ERROR: Could not get IID_IMediaFilter interface");
        }else{
    ...
    if (FAILED(hr)){
         if(verbose)LOG_ERROR("ERROR: Could not start graph");
         stopDevice(deviceID);
         return hr;
    }
    ...
    if(verbose)LOG_INFO("SETUP: Device is setup and ready to capture.");
...
//    printf("_index is %d.devices is %d.\n", _index, devices);
    int wanted_deviceNumber =0;
    if(_index==99){ /// index for SLAM cam
        for(int i =0; i<driver/devices;i++){
              printf("Camera num : %d, name : %s.\n", i, VI.deviceNames[i]);

            if(!strcmp(VI.deviceNames[i],"USB Camera-OV580")){
                wanted_deviceNumber=i;

                printf("xreal-Dolly SLAM camera found.\n");

                return open(wanted_deviceNumber);;
            }
        }
    }else if(_index==100){ /// index for world cam
          for(int i =0; i<driver/devices;i++){
            if(!strcmp(VI.deviceNames[i],"5M Camera")){
                wanted_deviceNumber=i;
                printf("xreal-Dolly World camera found.\n");
...
IplImage* CvCaptureCAM_DShow::retrieveFrame(int)
{
    ...
    if (VI.getPixels( index, (uint8_t*)frame->imageData, false, false ))
        return frame;
    else
        return NULL;
}
