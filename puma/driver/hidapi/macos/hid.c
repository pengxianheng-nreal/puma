...
static int init_hid_manager(hid_context *_context) {
    /* Initialize all the HID Manager Objects */
    *_context = IOHIDManagerCreate(kCFAllocatorDefault, kIOHIDOptionsTypeNone);
    if (*_context) {
        IOHIDManagerSetDeviceMatching(*_context, NULL);
        IOHIDManagerScheduleWithRunLoop(*_context, CFRunLoopGetCurrent(), kCFRunLoopDefaultMode);
        return 0;
    }
...
int HID_API_EXPORT hid_init(hid_context *_context) {
    if (*_context == NULL) {
        is_macos_10_10_or_greater =
            (NSAppKitVersionNumber >= 1343); /* NSAppKitVersionNumber10_10 */
        return init_hid_manager(_context);
    }
...
int HID_API_EXPORT hid_exit(hid_context *_context) {
    if (*_context) {
        ...
        IOHIDManagerClose(*_context, kIOHIDOptionsTypeNone);
        CFRelease(*_context);
        *_context = NULL;
    }
...
static struct hid_device_info *create_device_info_with_usage(IOHIDDeviceRef dev,
                                                             int32_t usage_page,
                                                             int32_t usage) {
    ...
    if (res == KERN_SUCCESS) {
        /* max value of entry_id(uint64_t) is 18446744073709551615 which is 20
           characters long, so for (max) "path" string
           'DevSrvsID:18446744073709551615' we would need 9+1+20+1=31 bytes
           byffer, but allocate 32 for simple alignment */
     ...
     * for its interface. */
    int is_usb_hid =
        get_int_property(dev, CFSTR(kUSBInterfaceClass)) == kUSBHIDClass;
    if (is_usb_hid) {
       ...
       int32_t interface_number = -1;
        if (try_get_int_property(dev, CFSTR(kUSBInterfaceNumber), &interface_number)) {
            ...
            cur_dev->interface_number = read_usb_interface_from_hid_service_parent(iokit_dev);
            HID_LOG_TRACE("read_usb_interface_from_hid_service_parent interface_number=%d",cur_dev->interface_number);
    ...
    HID_LOG_TRACE("cur_dev->interface_number=%d",cur_dev->interface_number);
    ...
    //         //temp_dev_path will like below:
    //         //IOService:/AppleARMPE/arm-io@10F00000/AppleT810xIO/usb-drd1@2280000/AppleT8103USBXHCI@01000000/usb-drd1-port-hs@01100000/Nreal Air@01100000/IOUSBHostInterface@5/AppleUserUSBHostHIDDevice
    //         //printf("\ntemp_dev_path=%s\n", temp_dev_path);
    //     }

    //     if (temp_dev_path) {
    //         HID_LOG_TRACE("old_interface_number=%d,temp_dev_path=%s",old_interface_number,temp_dev_path);
    ...
    //            //printf("\ncur_dev->interface_number=%d\n", cur_dev->interface_number);
    //             if (endptr == decimal_str) {
    //                //printf("\n The parsing failed. Set interface_number to old_interface_number:%d\n",old_interface_number);
...
static struct hid_device_info *create_device_info(IOHIDDeviceRef device) {
    const int32_t primary_usage_page =
        get_int_property(device, CFSTR(kIOHIDPrimaryUsagePageKey));
    const int32_t primary_usage =
        get_int_property(device, CFSTR(kIOHIDPrimaryUsageKey));

    /* Primary should always be first, to match previous behavior. */
    struct hid_device_info *root = create_device_info_with_usage(
        device, primary_usage_page, primary_usage);
    ...
    if (usage_pairs != NULL) {
        ...
        for (CFIndex i = 0; i < CFArrayGetCount(usage_pairs); i++) {
            ...
            if (!CFDictionaryGetValueIfPresent((CFDictionaryRef)dict,
                ...
                !CFNumberGetValue((CFNumberRef)usage_ref, kCFNumberSInt32Type,
                                  &usage)) {
                continue;
            }
...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(hid_context* _context, unsigned short vendor_id, unsigned short product_id, unsigned short interfaceNumber) {
    ...
    if (vendor_id != 0 || product_id != 0) {
        matching = CFDictionaryCreateMutable(
            kCFAllocatorDefault, kIOHIDOptionsTypeNone,
            &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);

        if (matching && vendor_id != 0) {
            CFNumberRef v = CFNumberCreate(kCFAllocatorDefault,
                                           kCFNumberShortType, &vendor_id);
            CFDictionarySetValue(matching, CFSTR(kIOHIDVendorIDKey), v);
            CFRelease(v);
        }

        if (matching && product_id != 0) {
            CFNumberRef p = CFNumberCreate(kCFAllocatorDefault,
                                           kCFNumberShortType, &product_id);
            CFDictionarySetValue(matching, CFSTR(kIOHIDProductIDKey), p);
            CFRelease(p);
        }
    }
    IOHIDManagerSetDeviceMatching(*_context, matching);
    if (matching != NULL) {
        CFRelease(matching);
    }

    CFSetRef device_set = IOHIDManagerCopyDevices(*_context);
    if (device_set == NULL) {
        return NULL;
    }
    ...
    HID_LOG_TRACE("num_devices=%ld",num_devices);
    ...
    for (i = 0; i < num_devices; i++) {
        ...
        HID_LOG_TRACE("current tmp,vid=%X,pid=%X,interface_number=%X,path=%s",tmp->vendor_id, tmp->product_id ,tmp->interface_number,tmp->path);
...
hid_device *HID_API_EXPORT hid_open(hid_context *_context, unsigned short vendor_id, unsigned short product_id, unsigned short interface_number, const wchar_t *serial_number) {
    /* This function is identical to the Linux version. Platform independent. */
    ...
    while (cur_dev) {
        if (cur_dev->vendor_id == vendor_id &&
            ...
            interface_number == cur_dev->interface_number) {
            if (serial_number) {
    ...
    if (path_to_open) {
        HID_LOG_TRACE("path_to_open=%s",path_to_open);
        /* Open the device */
...
/* \p path must be one of:
     ...
     - a valid path to an IOHIDDevice in the IOService plane (as returned by
   IORegistryEntryGetPath, e.g.:
   "IOService:/AppleACPIPlatformExpert/PCI0@0/AppleACPIPCI/EHC1@1D,7/AppleUSBEHCI/PLAYSTATION(R)3
   Controller@fd120000/IOUSBInterface@0/IOUSBHIDDriver"); Second format is for
   compatibility with paths accepted by older versions of HIDAPI.
 */

static io_registry_entry_t hid_open_service_registry_from_path(
    const char *path) {
...
hid_device *HID_API_EXPORT hid_open_path(hid_context *_context, const char *path) {
    ...
    /* Set up the HID Manager if it hasn't been done */
    if (hid_init(_context) < 0) goto return_error;
...
static int set_report(hid_device *dev, IOHIDReportType type, const unsigned char *data, size_t length) {
    ...
    //HID_LOG_TRACE("set_report enter,length=%zu",length);
    if (!data || (length == 0)) {
        HID_LOG_TRACE("set_report invalid params");
        return -1;
    }
    ...
    if (dev->disconnected) {
        HID_LOG_TRACE("set_report disconnected");
        return -1;
    }

    res = IOHIDDeviceSetReport(dev->device_handle, type, report_id,
                                   data_to_send, length_to_send);
    if (res == kIOReturnSuccess) {
        // Report was successfully set
        //HID_LOG_TRACE("Report set successfully.");
    } else {
        // There was an error
        HID_LOG_TRACE("IOHIDDeviceSetReport failed: (0x%08X) %s", res, mach_error_string(res));
    }
...
static int get_report(hid_device *dev, IOHIDReportType type,
    ...
    res = IOHIDDeviceGetReport(dev->device_handle, type, report_id, report,
                               &report_length);

    if (res == kIOReturnSuccess) {
        if (report_id ==
            0x0) { /* 0 report number still present at the beginning */
            report_length++;
        }
...
void HID_API_EXPORT hid_close(hid_device *dev) {
    ...
    */
    ...
    /* Close the OS handle to the device, but only if it's not
       ...
       Starting with macOS 10.15, there is an opposite effect in some
       environments: crash happenes if IOHIDDeviceClose() is not called. Not
       leaking a resource in all tested environments.
    */
    if (is_macos_10_10_or_greater || !dev->disconnected) {
